...
class RegressionModel():
    ...
    def buildModel(self, shape: Tuple[int, int]):
        n_features = 4

        self.model = Sequential()
        self.model.add(Input(shape=shape, name='input'))
        self.model.add(LSTM(self.config.lstm_neurons, return_sequences=False, name='lstm_hidden'))
    ...
    def trainModel(self, scaled_input_data: np.ndarray, scaled_target_data: np.ndarray) -> None:
        timestamp = time.time()
    ...
    def main(self):
        list_dataframes = self.generate_list()

        x_coordinates = list_dataframes[0][0].columns.astype(int).to_numpy()
        y_coordinates = list_dataframes[0][0].index.astype(int).to_numpy()
        ...
        for i in range(len(self.start_points)):
            for j in range(len(self.end_points)):
                self.linear_functions.append(self.__linear_interpolation(self.start_points[i], self.end_points[j]))

        print(len(self.linear_functions))

        x_grid, y_grid = np.meshgrid(x_coordinates, y_coordinates)
        coordinate_grid = np.stack([x_grid.ravel(), y_grid.ravel()], axis=-1)

        self.linear_functions = np.array(self.linear_functions)
        linear_functions_real = []
        for linear in self.linear_functions:
        ...
        linear_functions_real = np.array(linear_functions_real)

        print(linear_functions_real.shape)

        voltage_values = []

        for height in self.merged_df_list:
            for linear in linear_functions_real:
                temp_list = []
                for coord in linear:
        ...
        temp_linear_real = linear_functions_real
        linear_functions_real_full = []

        for i in range(0, len(self.merged_df_list)):
            for linear in temp_linear_real:
                linear_functions_real_full.append(linear[linear.shape[0]-1])

        linear_functions_real_full = np.array(linear_functions_real_full)
        ...
        scaled_target_data = scaler_target.transform(linear_functions_real_full)
        ...
        scaled_input_data = scaled_input_temp_full
        scaled_target_data = scaled_target_temp_full

        lr_scheduler = self.createScheduler(voltage_values.shape[0], inital_learning_rate=self.config.learning_rate, epochs=self.config.epochs)
        
        shape = (voltage_values.shape[1], voltage_values.shape[2])
        ...
        self.trainModel(scaled_input_data, scaled_target_data)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lstm_neurons', default=128, type=int, help='Define the number of neurons used in the lstm layers')
